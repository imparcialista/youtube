        ...
        if escolha == 'sair':
            ...
            if planilha_atualizar == '':
                ...
                if df_atualizar.columns[0] == 'SKU':
                    ...
                    elif df_atualizar.columns[1] == 'DES':
                        msg_cima('Modo atualizar promoções por planilha selecionado')
                        tipo_escolhido_planilha = 'desconto'
                        msg_alerta('ATENÇÃO: Produtos com promoção ativa não serão alterados para não sair da promoção')
                        planilha_des = True

                        for des_df in df_atualizar['DES']:
                            valor_trocar.append(des_df)
            ...
            for inx_cat, item in enumerate(retorno_cat):
                ...
                lista_retorno_cat.append([id_prd_cat, title_prd_cat, seller_id_cat, price_cat, endereco_seller])
